...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    result={'version':'110.3.21.8-segment-gentle-anatomical-layers','retargeting_mode':'fullbody_segment_frame_reference_skel_skeleton_layer','scale':float(scale.cpu()),
...
def _skel_volumetric_anatomy_2010(joints, joint_names):
    """V110.3.21.8 · Generates volumetric anatomically-shaped bone and muscle meshes.
...
def _plot_skel_mesh_walker(mesh_seq):
    """V110.3.21.8 · visor de atlas anatómico REAL sobre SKEL. Sin fallback geométrico."""
    try:
        ...
        if not _atlas_status.get("ready"):
            st.warning("V110.3.21.8 · Atlas BodyParts3D real todavía no está instalado en esta instancia.")
        ...
        # V110.3.21.8 · visor anatómico incremental.
        ...
        Plotly.newPlot(gd,data,{{
          title:'V110.3.21.8 · Anatomical Atlas Mesh',
        ...
        st.caption(
            "V110.3.21.8 usa directamente la superficie SKEL y los joints ya calculados. "
        ...
        st.markdown("---")
        st.markdown("### 🧭 Marco anatómico unificado · piel + joints + esqueleto")
        st.caption(
            "V110.3.21.8 usa un único marco corporal para piel, joints y esqueleto: "
            "superior = pelvis→tórax, medio-lateral = cadera izquierda→derecha y "
            "anteroposterior = producto vectorial. Las proporciones se fijan en el frame de referencia."
        )
        try:
            _J_u=np.asarray(mesh_seq.get("joints"),dtype=np.float32)
            _JN_u=[str(x) for x in mesh_seq.get("joint_names",[])]
            _V_u,_Jc_u,_orig_u,_axes_u=canonicalize_sequence(
                np.asarray(Vviz,dtype=np.float32),_J_u,_JN_u
            )
            _anth=anthropometric_summary(_J_u[0],_JN_u)
            st.success("Marco anatómico unificado calculado para los 75 frames sin rehacer el fitting.")
            st.dataframe(
                pd.DataFrame([{"Medida":kk,"Valor relativo 3D":vv} for kk,vv in _anth.items() if np.isfinite(vv)]).round(4),
                use_container_width=True,hide_index=True
            )
        except Exception as _e:
            st.warning(f"No se pudo construir el marco anatómico unificado: {type(_e).__name__}: {_e}")

        st.markdown("#### 🎬 Vídeos de marcha")
        st.caption(
            "Se mantienen los vídeos derivados de la secuencia SKEL existente. "
            "El vídeo Hamner recorre los 75 frames y no vuelve a procesar frontal/lateral."
        )
        _walk_mp4=st.session_state.get("v3218_hamner_walk_mp4")
        if _walk_mp4:
            try:
                with open(_walk_mp4,"rb") as _f:
                    _wb=_f.read()
                st.video(_wb)
                st.download_button(
                    "⬇️ Descargar vídeo Hamner · marcha",
                    _wb,
                    file_name="PhysioSentinel_Hamner_marcha_75frames.mp4",
                    mime="video/mp4",
                    use_container_width=True,
                    key="v3218_download_walk_top"
                )
            except Exception:
                pass

        st.markdown("#### 📊 Resultados y gráficas · Pestaña 12")
        st.caption(
            "La V110.3.21.8 conserva el FIX de la pestaña 12: en modo NPZ autónomo "
            "se calculan y muestran curvas articulares, ROM, derivadas, simetría, coordinación, "
            "diagramas ángulo–ángulo y trayectorias relativas 3D."
        )

        st.markdown("### 🧬 OpenSim · cuerpo completo Hamner 2010")
        st.caption(
            "V110.3.21.8 sustituye Rajagopal2016 como visor principal por FullBodyModel_Hamner2010_v2_0, "
            "que incluye miembros inferiores, tronco/cabeza y brazos simples. Las mallas de cada Body se fusionan y pelvis/sacro, manos y pies usan marcos anatómicos explícitos "
        ...
        if _hs.get("ready"):
            ...
            if st.button("🦴 Construir vista OpenSim · cuerpo completo",key="v3212_view_full",use_container_width=True):
                with st.spinner("Fusionando geometrías por Body y registrando cuerpo completo…"):
                    _hb=load_hamner_bodies(); _hp=build_hamner_fullbody_frame(_hb.get("bodies",{}),np.asarray(mesh_seq.get("joints"),dtype=np.float32),[str(x) for x in mesh_seq.get("joint_names",[])],frame=int(_hf)-1,skin_vertices=np.asarray(Vviz[int(_hf)-1],dtype=np.float32))
            ...
            if _hp:
                ...
                _fig.update_layout(title=f"OpenSim Hamner2010 · cuerpo completo · alineación cadera–fémur–rodilla + manos corregidas · frame {_hf}/{len(Vviz)}",height=780,margin=dict(l=0,r=0,t=45,b=0),scene=dict(aspectmode="data"))
        ...
        st.markdown("#### 🎥 Visores y descargas MP4")
        st.caption(
            "Los vídeos de Piel SKEL y Hamner se generan desde los 75 frames existentes. "
            "Ambos pueden visualizarse dentro de Streamlit y descargarse como archivos MP4."
        )
        st.markdown("#### ▶ Marcha Hamner · animación real de los 75 frames")
        st.caption(
            "El slider anterior sirve para inspeccionar un frame. Este bloque recorre automáticamente "
            "los 75 frames SKEL ya existentes y genera un MP4 continuo. No vuelve a ejecutar el fitting."
        )
        if _hs.get("ready") and st.button(
            "▶ Generar animación Hamner caminando · 75 frames",
            key="v3218_generate_hamner_walk",
            use_container_width=True
        ):
            with st.spinner("Renderizando la marcha Hamner desde los 75 frames ya calculados…"):
                _hb_anim=load_hamner_bodies()
                _mp4_path=render_hamner_walking_mp4(
                    _hb_anim.get("bodies",{}),
                    np.asarray(mesh_seq.get("joints"),dtype=np.float32),
                    [str(x) for x in mesh_seq.get("joint_names",[])],
                    skin_vertices_seq=np.asarray(Vviz,dtype=np.float32),
                    fps=25,
                    max_faces_per_body=500
                )
                st.session_state["v3218_hamner_walk_mp4"]=_mp4_path

        _mp4_saved=st.session_state.get("v3218_hamner_walk_mp4")
        if _mp4_saved:
            try:
                with open(_mp4_saved,"rb") as _f:
                    _mp4_bytes=_f.read()
                st.video(_mp4_bytes)
                st.download_button(
                    "⬇️ Descargar vídeo Hamner · marcha 75 frames",
                    data=_mp4_bytes,
                    file_name="PhysioSentinel_Hamner_marcha_75frames.mp4",
                    mime="video/mp4",
                    use_container_width=True,
                    key="v3218_download_hamner_walk"
                )
                st.success(
                    "Animación construida desde la secuencia SKEL existente: 75/75 frames. "
                    "Esto sí reproduce la marcha; no es una captura del slider."
                )
            except Exception as _e:
                st.warning(f"No se pudo abrir el MP4 Hamner generado: {type(_e).__name__}: {_e}")
        ...
        with st.expander("⚙️ Rajagopal2016 · rama anterior (solo referencia)",expanded=False):
            ...
            st.caption(
                "V110.3.21.8 inicia una rama biomecánica basada en Rajagopal2016/OpenSim. "
        ...
        # V110.3.21.8: legacy BodyParts3D video controls must not crash the OpenSim branch.
        ...
        with _vc1:
            ...
            if st.session_state.get("v2013_video_skin_bytes"):
                st.video(st.session_state["v2013_video_skin_bytes"])
        ...
        if not _atlas_status.get("ready"):
            ...
            st.caption("V110.3.21.8 · Render anatómico bajo demanda: sólo se carga la capa seleccionada para reducir memoria.")
    except Exception as exc:
...
def _load_mesh_sequence_npz_bytes_2012(raw):
    """V110.3.21.8 · Reabre un NPZ SKEL ya calculado sin volver a generar los 75 frames."""
    try:
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.caption("V110.3.21.8 · Carga el NPZ descargado de este registro para abrir directamente sus 75 frames SKEL. No repite fitting, retargeting ni generación de los 75 frames.")
