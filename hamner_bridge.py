...
def build_hamner_fullbody_frame(bodies,joints,joint_names,frame=0,skin_vertices=None):
    """V110.3.21.8 orientation-corrected Hamner full body.
    ...
    for body,p in bodies.items():
        ...
        if kind=="pelvis":
            ...
            # V110.3.21.8: register real mesh endpoint centroids exactly to SKEL joints.
            # This improves hip/femur/knee and knee/tibia/ankle continuity.
            W,_bone_scale=_register_long_bone_native(V,at,bt,P)
            ...
            # Hand size from forearm, slightly larger than .21.5 because it was nearly invisible.
            if hum is not None and ulna is not None:
                forearm=max(float(np.linalg.norm(J[0,hand]-J[0,ulna])),0.12)
            else:
                forearm=max(float(np.linalg.norm(J[0,hand]-J[0,rad])),0.12)

            target_len=float(np.clip(0.80*forearm,0.10,0.24))
            W=_constrained_hand_registration(V,J[frame,hand],T,target_len)
